...
for (const movie of starWarsMovies) {
    // Calculamos la década dividiendo el año entre 10, redondeando hacia abajo y multiplicando por 10
    const decade = Math.floor(movie.releaseYear / 10) * 10;

    // Si la década no existe como clave en el objeto, la inicializamos como un array vacío
    if (!moviesByDecade[decade]) {
        moviesByDecade[decade] = [];
    }

    // Añadimos la película al array de su década
